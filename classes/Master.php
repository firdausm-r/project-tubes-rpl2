...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Nama kategori yang dimaksud sudah ada.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Kategori baru berhasil ditambahkan.";
			else
				$resp['msg'] = " Kategori berhasil di ubah.";
	...
	function save_post(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Postingan Baru telah disimpan.";
			else
				$resp['msg'] = " Postingan Berhasil Diubah.";
	...
	function delete_post(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Post Berhasil Dihapus.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_comment(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Komentar Berhasil Ditambah.";
			else
				$resp['msg'] = " Komentar Berhasil Diubah.";
	...
	function delete_comment(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," Komentar Berhasil Dihapus.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
			...
			if(isset($comment_id)){
				...
				if(!empty($data)){
					...
					if(!$save_comments){
						...
						$resp['error'] = $this->conn->error;
						if(empty($id)){
							$resp['msg'] = "Gagal menyimpan transaksi.";
							$this->conn->query("DELETE FROM `transaction_comments` where transaction_id = '{$tid}'");
						}else{
							$resp['msg'] = "Gagal Update Transaksi.";
						}
						return json_encode($resp);
					}
				}
